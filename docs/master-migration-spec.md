...
| 4 — Framed Slave               | ✅ Verified                     | `BlockFramedSlave`/`TileFramedSlave`/`ItemFramedSlave` compile cleanly; toggle proxying to bound controller works; materials persist via `MatS`/`MatT`/`MatF` NBT                                                                                                                         |
| 5 — Rendering                  | 🟨 In progress (compile-passed) | Three `ISimpleBlockRenderingHandler` implementations compile (`FramedCompactDrawerRenderer`, `FramedControllerRenderer`, `FramedSlaveRenderer`); registered in `ClientProxy` with unique render IDs; dev client load and behavior pending                                                 |
| 6 — Recipes & Assets           | ⬜                              | All 3 recipes craft correct output; textures/lang/mcmod.info finalized, no missing-texture purple/black                                                                                                                                                                                   |
| 7 — Lifecycle Wiring & Cleanup | ✅ Verified                              | Full call-order audit clean: preInit→blocks/items/tiles, init→recipes+renderers; `Config.java` placeholder removed; stale comments cleaned; `gradlew build` + spotlessApply + check pass clean; smoke test of all 3 blocks confirmed working                                                                                                            |
